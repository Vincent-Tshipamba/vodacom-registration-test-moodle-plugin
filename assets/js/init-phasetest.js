window.testPhaseEditor = function (initialPhase) {
    return {
        phase: initialPhase,
        editing: {},
        values: {},
        ...
        init() {
            this.fields.forEach(field => {
                this.editing[field.name] = false;
                this.values[field.name] = this.phase[field.name] ?? '';
            });
        ...
        edit(field) {
            ...
            this.values[field] = this.phase[field] ?? '';
        },
        ...
        async save(field) {
            ...
            formData.append('value', this.values[field]);
            formData.append('editionid', this.phase.editionid);

            if (this.phase.id) {
                formData.append('id', this.phase.id);
            }
            ...
            if (!result.success) {
                Swal.fire('Erreur', result.message || 'Erreur serveur', 'error');
                return;
            }

            this.phase = result.phase;
            this.values[field] = result.phase[field] ?? '';
            this.editing[field] = false;
        ...
        statusClass() {
            if (this.phase.status === 'AWAITING') {
                return 'bg-yellow-100 text-yellow-700';
            }

            if (this.phase.status === 'IN_PROGRESS') {
                return 'bg-green-100 text-green-700';
            }

            if (this.phase.status === 'COMPLETED') {
                return 'bg-blue-100 text-blue-700';
            }

            if (this.phase.status === 'CANCELLED') {
